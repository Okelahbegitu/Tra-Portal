...
if (!empty($_GET['id_wisata'])) {
    ...
    mysqli_stmt_bind_param($stmt, "s", $_GET['id_wisata']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = mysqli_prepare(
            $conn,
            "UPDATE tbwisata 
             SET nama_wisata=?, lokasi=?, deskripsi_wisata=?, harga_wisata=? 
             WHERE id_wisata=?"
        );
        ...
        foreach ($oldAlbumArray as $old) {
            ...
            if (!in_array($oldId, $keepOldIds, true)) {
                ...
                $stmt = mysqli_prepare(
                    $conn,
                    "DELETE FROM album WHERE id_photo = ?"
                );
        ...
        if (!empty($_FILES['albumArray']['tmp_name'][0])) {
            ...
            foreach ($_FILES['albumArray']['tmp_name'] as $i => $tmp) {
                ...
                if ($_FILES['albumArray']['error'][$i] !== 0)
                    continue;
                ...
                $stmt = mysqli_prepare(
                    $conn,
                    "INSERT INTO album (id_wisata, id_photo, gambar)
                     VALUES (?, ?, ?)"
                );
                mysqli_stmt_bind_param($stmt, "sss", $idTour, $idPhoto, $pathDB);
                mysqli_stmt_execute($stmt);
            }
        }
        mysqli_commit($conn);
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2 text-5xl font-semibold mb-2 flex items-center gap-2">
                            ...
                            <div class="border-2 w-full border-palette-3  rounded-lg px-3 py-2 flex items-center gap-2 "
                                ...
                                <input type="file" name="albumArray[]" multiple class="hidden" id="pict">
